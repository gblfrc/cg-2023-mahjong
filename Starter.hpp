...
protected:
	...
	VkImageSubresource entitySubresource = { VK_IMAGE_ASPECT_COLOR_BIT,	0,	0};
	VkSubresourceLayout entityLayout;
...
//		createInfo.ppEnabledExtensionNames = glfwExtensions;
	...
	void getSixAxis(float &deltaT, glm::vec3 &m, glm::vec3 &r, bool &fire, bool &click, bool &enter, bool &mButton) {
		...
		if (glfwGetKey(window, GLFW_KEY_M)) {		//Press M key
			mButton = true;
		}
...
void Texture::initFive(BaseProject* bp, const char* files[5]) {
	BP = bp;
	imgs = 5;
	createTextureImage(files);
	createTextureImageView();
